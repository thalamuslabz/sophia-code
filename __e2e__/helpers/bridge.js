...
export class AutoClaudeBridge {
    ...
    async createACSpec(spec) {
        ...
        await fs.writeFile(path.join(acSpecDir, 'source-spec.json'), JSON.stringify(spec, null, 2));
        this.log('INFO', `Created AC spec at: ${acSpecDir}`);
    }
    inferWorkflowType(spec) {
    ...
    inferServices(spec) {
        const services = [];
        if (spec.tech_stack?.includes('react'))
            services.push('frontend');
        if (spec.tech_stack?.includes('node') || spec.tech_stack?.includes('python'))
            services.push('backend');
        if (spec.tech_stack?.includes('postgres') || spec.tech_stack?.includes('mongo'))
...
`;
    ...
    generateImportInstructions(spec, specDirName) {
        return `==================================
