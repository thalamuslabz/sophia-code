...
export class EvidenceVault {
  ...
  async start(): Promise<void> {
    ...
    this.watcher.on('add', (filePath: string) => this.handleNewEvidence(filePath));
    this.watcher.on('change', (filePath: string) => this.handleNewEvidence(filePath));
   ...
   */
  async syncEvidence(intentId: string): Promise<void> {
  ...
  private async handleNewEvidence(filePath: string): Promise<void> {
  ...
  private async syncToObsidian(evidence: Evidence): Promise<void> {
    const projectDir = this.sanitizeProjectName(evidence.project);
    const obsidianProjectDir = path.join(
      this.config.obsidianVaultDir,
      this.config.projectsSubdir,
      this.inferProjectCode(evidence.project)
    );
  ...
  private generateObsidianNote(evidence: Evidence): string {
