...
interface EvidenceCaptureConfig {
  acOutputDir: string;
  evidenceDir: string;
  sophiaApiUrl: string;
}
...
export class EvidenceCapture {
  private config: EvidenceCaptureConfig;
  private watchers: Map<string, FSWatcher> = new Map();
  private activeCaptures: Map<string, Evidence> = new Map();

  constructor(config: Partial<EvidenceCaptureConfig> = {}) {
   ...
   */
  async startCapture(intentId: string, specId: string, project: string): Promise<void> {
   ...
   */
  async stopCapture(intentId: string): Promise<Evidence | null> {
   ...
   */
  async captureArtifacts(intentId: string): Promise<void> {
    const evidence = this.activeCaptures.get(intentId);
    if (!evidence) return;

    const projectDir = path.join(
      this.config.acOutputDir,
   ...
   */
  async getStats(intentId: string): Promise<{ files: number; lastActivity: string | null }> {
    const evidence = this.activeCaptures.get(intentId);
    if (!evidence) {
  ...
  private async setupWatcher(intentId: string, specId: string, project: string, projectDir: string): Promise<void> {
  ...
  private async watchForProjectCreation(intentId: string, specId: string, project: string, projectDir: string): Promise<void> {
    ...
    const checkAndSetup = async (createdPath: string) => {
  ...
  private async handleFileChange(intentId: string, filePath: string, projectDir: string): Promise<void> {
    const evidence = this.activeCaptures.get(intentId);
    if (!evidence) return;
  ...
  private classifyArtifact(filePath: string): Artifact['type'] {
  ...
  private async captureTestResults(projectDir: string): Promise<TestResults | undefined> {
  ...
  private async reportToSophia(evidence: Evidence): Promise<void> {
    try {
